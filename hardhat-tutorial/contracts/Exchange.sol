...
contract Exchange is ERC20 {
    ...
    function addLiquidity(uint _amount) public payable returns (uint) {
        ...
        if (cryptoDevTokenReserve == 0) {
            ...
            // _mint is a ERC20.sol smart contract function to mint ERC20 tokens
        } else {
            /**
             * If the reserve is not empty, intake any user supplied value for
             * `Ether` and determine according to the ratio how many `Crypto Dev` tokens
             * needs to be supplied to prevent any price impacts because of the additional liquidity
             */
